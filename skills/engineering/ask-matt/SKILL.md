...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then runs **`/code-review`**, a three-axis review (Standards + Spec + Adversarial) of the diff. The adversarial axis treats every author-supplied claim as untrusted and tries to break the change, while requiring concrete evidence for every finding. Finally, **`/verifying-work`** drives the finished change through its real user-facing surface and hands back reproducible proof; only `VERIFIED` reaches the commit. If the repository has no safe verification harness, it stops `INCONCLUSIVE`: run **`/building-verification`** once, then retry the proof. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, **`/code-review`** when you want to review a branch or PR against a fixed point, and **`/verifying-work`** when the question is whether completed work demonstrably functions.

   Publishing stays explicit and outside `/implement`: ask for **`/file-pr`** when the committed branch should become a concise pull request, and add **`/babysit-pr`** when it should also be watched through review and CI. Filing and monitoring are separate because either can be requested without the other; both stop for approval before external communication.
...
- **`/html-communication`** — present a plan, spec, findings, report, comparison, or static UI mock set as one readable HTML artifact. It is for communicating a document outside the terminal; `/prototype` owns artifacts the user must drive to answer a behavioural or design question.
